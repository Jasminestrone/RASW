...
def calculate_fk(arm_lengths: List[float], joint_angles: List[float]) -> Tuple[List[Tuple[float, float]], Optional[str]]:
    ...
    # Calculate cumulative angle at each joint
    cumulative_angle = 0
    ...
    for i in range(len(arm_lengths)):
        ...
        arm_vector = np.array([arm_lengths[i], 0])
